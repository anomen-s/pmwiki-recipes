...
/***************************************************************************
 * wikilog.php, version 1.1.3 requires PmWiki 2.2.56 or above              *
 * This is a calendar display and wikilog macro for the PmWiki engine      *
 * Copyright (c) 2003-20014 John Rankin john.rankin@affinity.co.nz         *
 *                                                                         *
...
function view_calendar($pagename,$homepage) {
	...
	if (@$subtitle) 
		return '(:title '.$AsSpacedFunction($subtitle).':)'.
			(IsDate($title) ? "<h3>[[$title |".longdate($title)."]]</h3>" : '');
	elseif (IsDate($title) && ($summary_only==true)) {
...
function view_calendar_list($pagename,$homegroup,$hometitle) {
	...
	if ($enable_publish) {
    	$PublishCalendarFmt = str_replace(array('$Month','$Year'),
    	   array($month,"$homegroup.$year"),$PublishCalendarFmt);
        $r .= '(:publishcalendar:)<p />';
    }
	return "$r";
}
...
function list_entries($month, $year, $group) {
	...
	for ($i=1;$i<=$last_day_of_month;$i++) {
		...
		$firstpara = preg_replace_callback("/\\[\\=(.*?)\\=\\]/s",
		    function ($m) { return Keep($m[1]); },
                    $firstpara);
		if ($calendar_subpages) 
		    $firstpara = str_replace('[[,',"[[($entryname,)",$firstpara);
			}
			$line.=FmtPageName($firstpara,$entryname) . "</li>";
...
function auditWikilog() {
  global $MaxWikilogLinks;
  if (!(@$_POST['access'] && ($_POST['access']==$_POST['accesscode']) && 
        @$_POST['post'])) return false;
  SDV($MaxWikilogLinks, 3);
  preg_match_all('/https?:/',$_POST['text'],$match);
  return ((count($match[0])>$MaxWikilogLinks) ? false : true);
}
...
function HandleWikilogPost($pagename) {
   ...
   if (@$_POST['subpage']) {
       SDV($RecentChangesFmt['$Group.$EventDate'], 
          '* [['.($calendar_subpages ? ',' : "($date-)").
          '$ChangeSummary]]   `.. . . $[submitted] $CurrentTime $[by] $AuthorLink');
       SDV($ParentDateFmt, '($[subpage of] [[$Group/'.$date.']])');
       $RecentChangesFmt['$Group.RecentChanges'] = str_replace('. . .', 
           '. . . '.$ParentDateFmt,$RecentChangesFmt['$Group.RecentChanges']);
       $_POST['csum'] = FmtPageName('$Namespaced',
          MakePageName($pagename, $_POST['csum']));
       $FmtV['$EventDate'] = $date;
       $date .= ($calendar_subpages ? ',' : '-').$_POST['csum'];
   } else {
       $name = @$_POST['author'];
